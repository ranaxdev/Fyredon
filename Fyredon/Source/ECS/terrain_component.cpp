...
#include "Fyredon/ECS/terrain_component.h"
#include "Fyredon/ECS/terrain_entity.h"
// the parent of this component must contain mesh_component
terrain_component::terrain_component(terrain_entity* parent)
{
	this->componentName = "Terrain";
	this->parent = parent;
	...
	this->memmory_size = sizeof(terrain_component);
}

void terrain_component::GUI_struct()
{
	//refreshDetail();
	ImGui::InputInt("Resolution", &this->res);
	ImGui::InputText("File Name", &this->fileName, 0, NULL, NULL);
	if (ImGui::Button("Redraw Mesh"))
	{
		setResolution();
		setFileName();
		recreateMesh();
...
void terrain_component::setResolution() {
	//parent->res = this->res;
}

void terrain_component::setFileName() {
	//parent->fileName = fileName;
}



void terrain_component::recreateMesh()
{
	//parent->
...
//		this->min_X = this->max_X = this->mesh->vert_data[0].pos[0];
//		this->max_X = this->max_X = this->mesh->vert_data[0].pos[0];
//		this->min_Z = this->max_X = this->mesh->vert_data[0].pos[2];
//		this->max_Z = this->max_X = this->mesh->vert_data[0].pos[2];
//	}
//	for (int i = 0; i < this->mesh->vert_count; i++) {
