...
#include "Fyredon/FBX/FBXLoader.h"

FBXLoader::FBXLoader(const char* pFileName)
	...
	mPoseIndex(-1), SupportVBO(true)
{
	...
	if (SdkManager)
	{
		...
		{
			std::cout << "FBX binary (*.fbx)" << std::endl;
	...
	std::cout << WindowMessage << std::endl;
}
...
bool FBXLoader::Draw()
{
	...
	std::cout << SelectedNode << std::endl;
	if (SelectedNode)
	{
		glScalef(5, 5, 5);
...
bool VBOMesh::Initialize(const FbxMesh* pMesh)
{
		...
		if (lMaterialIndice && lMaterialMappingMode == FbxGeometryElement::eByPolygon)
		{
			...
			if (lMaterialIndice->GetCount() == lPolygonCount)
			{
				...
				for (int lPolygonIndex = 0; lPolygonIndex < lPolygonCount; ++lPolygonIndex)
				{
					...
					if (lMaterialIndex >= 0) {	// material exist
	...
	//std::cout << "lPolygonVertexCount: " << lPolygonVertexCount << std::endl;
	//std::cout << "mAllByControlPoint: " << mAllByControlPoint << std::endl;
	//std::cout << "lPolygonCount: " << lPolygonCount << std::endl;
	this->verticesCount = lPolygonVertexCount;
	this->lVerticesLength = lPolygonVertexCount * VERTEX_STRIDE;
	this->lVertices = new float[this->lVerticesLength];
	unsigned int* lIndices = new unsigned int[lPolygonCount * TRIANGLE_VERTEX_COUNT];
	this->lNormals = NULL;
	if (mHasNormal)
	{
		this->lNormalsLength = lPolygonVertexCount * NORMAL_STRIDE;
		this->lNormals = new float[this->lNormalsLength];
	}
	this->lUVs = NULL;
	...
	{
		this->lUVsLength = lPolygonVertexCount * UV_STRIDE;
		this->lUVs = new float[this->lUVsLength];
		lUVName = lUVNames[0];
	}

	// Populate the array with vertex attribute, if by control point.
	const FbxVector4* lControlPoints = pMesh->GetControlPoints();
	const int* lPolygon = pMesh->GetPolygonVertices();
	...
	if (mAllByControlPoint)
	{
		...
		for (int lIndex = 0; lIndex < lPolygonVertexCount; ++lIndex)
		{
			...
			this->lVertices[lIndex * VERTEX_STRIDE] = static_cast<float>(lCurrentVertex[0]);
			this->lVertices[lIndex * VERTEX_STRIDE + 1] = static_cast<float>(lCurrentVertex[1]);
			this->lVertices[lIndex * VERTEX_STRIDE + 2] = static_cast<float>(lCurrentVertex[2]);
			this->lVertices[lIndex * VERTEX_STRIDE + 3] = 1;
			// Save the normal.
			if (mHasNormal)
			{
				...
				this->lNormals[lIndex * NORMAL_STRIDE] = static_cast<float>(lCurrentNormal[0]);
				this->lNormals[lIndex * NORMAL_STRIDE + 1] = static_cast<float>(lCurrentNormal[1]);
				this->lNormals[lIndex * NORMAL_STRIDE + 2] = static_cast<float>(lCurrentNormal[2]);
			}
			...
			if (mHasUV)
			{
				...
				this->lUVs[lIndex * UV_STRIDE] = static_cast<float>(lCurrentUV[0]);
				this->lUVs[lIndex * UV_STRIDE + 1] = static_cast<float>(lCurrentUV[1]);
			}
		}
	}
	else {// by polygon
		...
		{
			// Save the vertex position.
			lCurrentVertex = lControlPoints[lPolygon[lIndex]];
			this->lVertices[lIndex * VERTEX_STRIDE] = static_cast<float>(lCurrentVertex[0]);
			this->lVertices[lIndex * VERTEX_STRIDE + 1] = static_cast<float>(lCurrentVertex[1]);
			this->lVertices[lIndex * VERTEX_STRIDE + 2] = static_cast<float>(lCurrentVertex[2]);
			this->lVertices[lIndex * VERTEX_STRIDE + 3] = 1;
	...
	for (int lPolygonIndex = 0; lPolygonIndex < lPolygonCount; ++lPolygonIndex)
	{
		...
		if (lMaterialIndex >= 0) {// meterial exist
		// Where should I save the vertex attribute index, according to the material
			...
			for (int lVerticeIndex = 0; lVerticeIndex < TRIANGLE_VERTEX_COUNT; ++lVerticeIndex)
			{
				...
				else
				{
					...
					this->lVertices[lVertexCount * VERTEX_STRIDE] = static_cast<float>(lCurrentVertex[0]);
					this->lVertices[lVertexCount * VERTEX_STRIDE + 1] = static_cast<float>(lCurrentVertex[1]);
					this->lVertices[lVertexCount * VERTEX_STRIDE + 2] = static_cast<float>(lCurrentVertex[2]);
					this->lVertices[lVertexCount * VERTEX_STRIDE + 3] = 1;
					/*
					std::cout << lCurrentVertex[0]
						<< lCurrentVertex[1]
						<< lCurrentVertex[2] << std::endl;*/
					if (mHasNormal)
					{
						pMesh->GetPolygonVertexNormal(lPolygonIndex, lVerticeIndex, lCurrentNormal);
						this->lNormals[lVertexCount * NORMAL_STRIDE] = static_cast<float>(lCurrentNormal[0]);
						this->lNormals[lVertexCount * NORMAL_STRIDE + 1] = static_cast<float>(lCurrentNormal[1]);
						this->lNormals[lVertexCount * NORMAL_STRIDE + 2] = static_cast<float>(lCurrentNormal[2]);
					}
					...
					if (mHasUV)
					{
						...
						this->lUVs[lVertexCount * UV_STRIDE] = static_cast<float>(lCurrentUV[0]);
						this->lUVs[lVertexCount * UV_STRIDE + 1] = static_cast<float>(lCurrentUV[1]);
					}
				}
				++lVertexCount;
			}
